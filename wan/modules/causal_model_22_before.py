        ...
        if kv_cache is None:
            ...
            if self.local_attn_size != -1 and (current_end > kv_cache["global_end_index"].item()) and (
                ...
                if not self._printed_once:
                    print(f"[KV rolling] sink={sink_tokens // frame_seqlen} frames, "
                          f"evict={num_evicted_tokens // frame_seqlen} frames, "
                          f"rolled={num_rolled_tokens // frame_seqlen} frames, "
                          f"kv_cache_total={kv_cache_size // frame_seqlen} frames")
            ...
            if sink_tokens > 0:
                ...
                if local_budget > 0:
                    ...
                    if not getattr(self, '_printed_attn_once', False):
                        print(f"[Attention] sink={sink_tokens // frame_seqlen} frames + "
                              f"local={k_local.shape[1] // frame_seqlen} frames = "
                              f"total={k_cat.shape[1] // frame_seqlen} frames in attention")
                        self._printed_attn_once = True
                else:
                    k_cat = k_sink
                    v_cat = v_sink
                x = attention(
