...
class WanVAEWrapper(torch.nn.Module):
    ...
    def decode_to_pixel_chunk(self, latent: torch.Tensor, use_cache: bool = False, chunk_size: int = 120, fps: int = 16):
        zs = latent.permute(0, 2, 1, 3, 4)
        if use_cache:
            assert latent.shape[0] == 1, "Batch size must be 1 when using cache"
        ...
        for u in zs:
            ...
            if num_frames <= chunk_size:
                ...
                for start_idx in range(0, num_frames, chunk_size):
                    print(f'decode from start_idx {start_idx}')
                    ...
                    frames = decoded_chunk.permute(1, 2, 3, 0)
                    frames = ((frames * 0.5 + 0.5).clamp(0, 1) * 255).byte().cpu().numpy()
                    if proc is None:
                        _, H_out, W_out, _ = frames.shape
                        cmd = [
                            'ffmpeg', '-y',
                    ...
                    del decoded_chunk, frames
                    torch.cuda.empty_cache()
                proc.stdin.close()
                proc.wait()
                if proc.returncode != 0:
                    raise RuntimeError(f'ffmpeg failed with returncode {proc.returncode}')
                self.model.clear_cache()
                output.append(VideoPath(tmp_video_path))

        if any(isinstance(o, VideoPath) for o in output):
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        self.seq_len = get_seq_frame_len(return_seq=False, return_frame=True, caller=str(self.__class__)) * local_attn_size if local_attn_size > 21 else get_seq_frame_len(return_seq=True, return_frame=False, caller=str(self.__class__))
        self.post_init()
