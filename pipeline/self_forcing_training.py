...
from utils.global_config import get_seq_frame_len, get_wan_version
...
class SelfForcingTrainingPipeline:
    def __init__(self,
        ...
        self.sink_size = int(kwargs.get("sink_size", 1))
        self.whether_set_per_block_sink_size = kwargs.get("whether_set_per_block_sink_size", False)
        self.sink_size_per_block = kwargs.get("sink_size_per_block", [self.sink_size] * self.num_transformer_blocks)
    ...
    def _set_per_block_sink_size(self, sink_size_per_block: list):
        ...
        if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
            print(f"[SelfForcingTrainingPipeline] per-block sink_size set: {sink_size_per_block}")
    ...
    def generate_chunk_with_cache(
        ...
        *,
        ...
        for block_index, current_num_frames in enumerate(all_num_frames):
            ...
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0) and block_index == 0:
    ...
    def inference_with_trajectory(
        ...
        if self.whether_set_per_block_sink_size:
            self._set_per_block_sink_size(self.sink_size_per_block)
        else:
        ...
        for block_index, current_num_frames in enumerate(all_num_frames):
            ...
            for index, current_timestep in enumerate(self.denoising_step_list):
                ...
                if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                    print(f"rank {dist.get_rank() if dist.is_initialized() else 0}, current_start_frame: {current_start_frame}, current_num_frames: {current_num_frames}, current_timestep: {current_timestep}")
                if not exit_flag:
                    with torch.no_grad():
                        _, denoised_pred = self.generator(
                            noisy_image_or_video=noisy_input,
            ...
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(f"rank {dist.get_rank() if dist.is_initialized() else 0}, current_start_frame: {current_start_frame}, current_num_frames: {current_num_frames}, current_timestep: {current_timestep}")
                print(f"rank {dist.get_rank() if dist.is_initialized() else 0}, rerun_for_cache")
            with torch.no_grad():
                self.generator(
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device):
        ...
        kv_cache1 = []
        if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
            print(f"rank {dist.get_rank() if dist.is_initialized() else 0} initialize kv cache with batch_size: {batch_size}, kv_cache_size: {self.kv_cache_size}")
