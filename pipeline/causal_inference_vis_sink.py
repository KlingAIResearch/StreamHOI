    ...
    def inference(
        ...
        for current_num_frames in tqdm(all_num_frames, desc=f"rank {dist.get_rank() if dist.is_initialized() else 0}"):
            ...
            if visualize_local_attn:
                _causal_model_22_mod.LOCAL_ATTN_VIS_ENABLED = (chunk_loop_id >= 4)

            # Only collect attention for chunk 0 (first generated chunk)
            _causal_model_22_mod.SINK_ATTN_VIS_ENABLED = visualize_sink_attn and (chunk_loop_id == 0)
            _causal_model_22_mod.CROSS_ATTN_VIS_ENABLED = visualize_cross_attn and (chunk_loop_id == 0)
            if visualize_local_attn:
                _causal_model_22_mod.LOCAL_ATTN_VIS_ENABLED = (chunk_loop_id == 1)
            ...
            if visualize_sink_attn and sink_frames_rgb is not None and chunk_loop_id == 0:
                ...
                if len(_causal_model_22_mod.SINK_ATTN_SCORE_BUFFER) > 0:
                    buffer_save_path = os.path.join(vis_output_dir, "sink_attn_buffer_chunk00.pt")
            ...
            if visualize_local_attn and chunk_loop_id >= 4:
            ...
            if not first_chunk_done and target_sink_size > 1:
                ...
                if not dist.is_initialized() or dist.get_rank() == 0:
                    print(f"[inference] sink_size switched to {target_sink_size} after first chunk")
