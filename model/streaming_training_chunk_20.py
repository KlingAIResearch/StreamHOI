...
class StreamingTrainingModel:
    ...
    def generate_next_chunk(self, requires_grad: bool = True) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        if previous_frames is not None:
            # overlap=0: always generate chunk_size new frames, no overlap
            overlap_frames_to_use = 0
            new_frames_to_generate = self.chunk_size

            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(
                    f"[StreamingTrain-Model] overlap=0: "
                    f"generating {new_frames_to_generate} new frames"
                )
        else:
            # First chunk: no initial_latent prepended, generate chunk_size frames directly
            overlap_frames_to_use = 0
            new_frames_to_generate = self.chunk_size
            ...
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(
                    ...
                    f"{new_frames_to_generate} frames (no initial latent prepended)"
                )
        ...
        # overlap=0: full_chunk is just the generated frames, no bridge/initial_latent prepended
        ...
        # gradient_mask: all frames supervised
