...
class DMD(SelfForcingModel):
    ...
    def _compute_kl_grad(
        ...
        return grad, {
            ...
            "_debug_pred_fake": pred_fake_image.detach(),
            "_debug_pred_real": pred_real_image.detach(),
        }
    ...
    def compute_distribution_matching_loss(
        ...
        debug_current_length: Optional[int] = None,
    ) -> Tuple[torch.Tensor, dict]:
        """
        ...
        with torch.no_grad():
            ...
            if self.independent_first_frame:
                noisy_latent[:, 0] = image_or_video[:, 0]
        ...
        if debug_step is not None:
            pred_fake = dmd_log_dict.pop("_debug_pred_fake")
            pred_real = dmd_log_dict.pop("_debug_pred_real")
            self._debug_save_latents(original_latent, pred_fake, pred_real, debug_step, debug_current_length)
        else:
            dmd_log_dict.pop("_debug_pred_fake", None)
            dmd_log_dict.pop("_debug_pred_real", None)

        return dmd_loss, dmd_log_dict

    def _debug_save_latents(self, original_latent, pred_fake, pred_real, step, current_length):
        rank = dist.get_rank() if dist.is_initialized() else 0
        save_dir = os.path.join("vis", "debug_long_training_all_steps_overlap5")
        os.makedirs(save_dir, exist_ok=True)

        def latent_to_video_tensor(latent):
            with torch.no_grad():
                pixels = self.vae.decode_to_pixel(latent.to(dtype=self.dtype))
            pixels = pixels.float().clamp(-1, 1)
            import numpy as np
            video_np = ((pixels[0].cpu().numpy() + 1) / 2 * 255).astype("uint8")
            video_np = video_np.transpose(0, 2, 3, 1)
            return torch.from_numpy(video_np)

        prefix = "critic_" if pred_real is None else ""
        items = [("generated", original_latent), ("pred_fake", pred_fake)]
        if pred_real is not None:
            items.append(("pred_real", pred_real))

        for name, latent in items:
            try:
                video_tensor = latent_to_video_tensor(latent)
                fname = f"rank{rank:02d}_step{step:06d}_len{current_length:04d}_{prefix}{name}.mp4"
                out_path = os.path.join(save_dir, fname)
                write_video(out_path, video_tensor, fps=16)
            except Exception as e:
                print(f"[DebugSave] Failed to save {name}: {e}")
    ...
    def critic_loss(
        ...
        if self.independent_first_frame:
            noisy_generated_image[:, 0] = generated_image[:, 0]
